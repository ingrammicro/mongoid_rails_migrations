  ...
  class Migrator#:nodoc:
    class << self
      ...
      def status(migrations_path, target_version = nil)
        case
          when target_version.nil?              then new(:up, target_version).status
          when current_version > target_version then new(:down, target_version).status
          else                                       new(:up, target_version).status
        end
      end
